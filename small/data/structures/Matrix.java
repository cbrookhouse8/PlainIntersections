...
public class Matrix {
	...
	public Matrix(float[] a, int _n, int _m) {
		this.n = _n;
		this.m = _m;
		this.M = new float[n * m];
		for (int i = 0; i < n * m; i++) {
	...
	Matrix(float angle, Matrix axis, int dim) {
		...
		Matrix B = new Matrix(
				new float[] { 0, axis.M[2], -axis.M[1], -axis.M[2], 0, axis.M[0], axis.M[1], -axis.M[0], 0 }, 3, 3);
		...
		for (int j = 0; j < 3; j++) {
			for (int i = 0; i < 3; i++) {
				C.M[j * C.n + i] = axis.M[i] * axis.M[j];
			}
		}

		A.scale(Math.cos(angle));
		B.scale(Math.sin(angle));
		C.scale(1 - Math.cos(angle));
	...
	public Matrix mult(Matrix A) {
		...
		if (A.m != this.n) {
			log.info("inner dimensions don't agree");
		} else {
	...
	// find a-b
	public Matrix matrixSub(Matrix a, Matrix b) {
	...
	public Matrix cross(Matrix Q) {
		...
		if (Q.M.length == 3 && this.M.length == 3) {
			Matrix rval = new Matrix(new float[] { -this.M[2] * Q.M[1] + this.M[1] * Q.M[2],
					this.M[2] * Q.M[0] + -this.M[0] * Q.M[2], -this.M[1] * Q.M[0] + this.M[0] * Q.M[1] }, 3, 1);
			return rval;
		}
	...
	// only for 3-Vectors
	public Matrix principalComponents(Matrix Pts) {
		 ...
		 * 
		 * for (int j = 0 ; j < Pts.n ; j++) { for (int i = 0 ; i < Pts.m ; i++) {
		...
		for (int cj = 0; cj < 3; cj++) {
			for (int ci = 0; ci <= cj; ci++) { // ci<=cj because C is symmetric
				...
				C[cj * 3 + ci] = val;
				if (ci != cj)
					C[ci * 3 + cj] = val; // because C is symmetric
			}
		}
	...
	public void normalize() {
		float mag = (float) Math.pow(this.dot(this), 0.5f);
		for (int i = 0; i < this.M.length; i++)
			this.M[i] /= mag;
	}

	public float mag() {
		return (float) Math.pow(this.dot(this), 0.5f);
	}

	public Matrix project(int row) {
		for (int j = 0; j < m; j++) {
			for (int i = 0; i <= row; i++)
				M[j * n + i] = M[j * n + i] / M[j * n + row];
		}
		return this;
	}
	...
	Matrix CalculateEigenSystem(float[] _m) { // for a 3x3 symmetric matrix _m
		int maxSweeps = 32;
		float epsilon = 0.00004539992f; // check this is the val of 1e^-10
		...
		for (int a = 0; a < maxSweeps; a++) {
			...
			if (((Math.abs(m12) < epsilon) && (Math.abs(m13) < epsilon)) && (Math.abs(m23) < epsilon)) {
				log.info("System calculated in " + a + " sweeps");
				a = maxSweeps;
			}

			// Annihilate (1,2) entry.
			if (m12 != 0) {
				float u = (m22 - m11) * 0.5f / m12; // Equation 16.46 in Lengyel. Derived from (16.43):
				...
				float t = (u2p1 != u2) ? ((u < 0 ? -1 : 1)  * ((float) (Math.pow(u2p1, 0.5f) - Math.abs(u))) ) : 0.5f / u;

				// find cos(theta) using t^2 + 1 = 1/cos(theta)^2
				float c = (float) (1 / Math.pow(t * t + 1, 0.5f)); // cos(theta)
			...
			if (m13 != 0) {
				...
				float u2p1 = u2 + 1;
				float t = (u2p1 != u2) ? ((u < 0) ? -1 : 1) * ((float) (Math.pow(u2p1, 0.5f) - Math.abs(u))) : 0.5f / u;
				float c = (float) (1 / Math.pow(t * t + 1, 0.5f));
				float s = c * t;
