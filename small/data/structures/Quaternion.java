...
public class Quaternion {
	...
	public Quaternion(float[] _from, float[] _to) {
		v = new float[3];
		...
		float magF = (float) Math.pow(this.dot(_from, _from), 0.5f);
		float magT = (float) Math.pow(this.dot(_to, _to), 0.5f);
		...
		this.axisAngleInit((float) Math.acos(this.dot(from, to)), this.cross(from, to));
	}
	...
	public Quaternion(float theta, float[] axis) {
		v = new float[3];
		this.axisAngleInit(theta, axis);
	}

	public Quaternion(float theta, float p1, float p2, float p3) {
		v = new float[3];
		this.axisAngleInit(theta, new float[] { p1, p2, p3 });
	}
	
	public Quaternion(double theta, double p1, double p2, double p3) {
		v = new float[3];
		this.axisAngleInit((float) theta, new float[] { (float) p1, (float) p2, (float) p3 });
	}
	
	// rotation matrix to quaternion
	public Quaternion(Matrix A) {
		s = 0;
		...
		// -r11-r22+r33 == 4z^2 - 1
		if (A.M.length == 16) {
			s = (float) Math.pow(A.M[0] + A.M[5] + A.M[10] + 1, 0.5f) / 2;
			this.v[0] = (float) Math.pow(A.M[0] - A.M[5] - A.M[10] + 1, 0.5f) / 2;
			this.v[1] = (float) Math.pow(-A.M[0] + A.M[5] - A.M[10] + 1, 0.5f) / 2;
			this.v[2] = (float) Math.pow(-A.M[0] - A.M[5] + A.M[10] + 1, 0.5f) / 2;
		} else if (A.M.length == 9) {
			this.s = (float) Math.pow(A.M[0] + A.M[4] + A.M[8] + 1, 0.5f) / 2;
			this.v[0] = (float) Math.pow(A.M[0] - A.M[4] - A.M[8] + 1, 0.5f) / 2;
			this.v[1] = (float) Math.pow(-A.M[0] + A.M[4] - A.M[8] + 1, 0.5f) / 2;
			this.v[2] = (float) Math.pow(-A.M[0] - A.M[4] + A.M[8] + 1, 0.5f) / 2;
		}
	}

	void axisAngleInit(float theta, float[] axis) {
		this.s = (float) Math.cos(theta / 2);

		float mag = 0;

		for (int i = 0; i < 3; i++)
			v[i] = axis[i];
		mag = (float) Math.pow(this.dot(axis, axis), 0.5f);

		for (int i = 0; i < 3; i++) {
			this.v[i] /= mag; // normalize
			this.v[i] *= (float) Math.sin(theta / 2);
		}
	}
	...
	public Quaternion mult(Quaternion q) {
		...
		for (int i = 0; i < 3; i++)
			this.v[i] = v1Xv2[i] + this.s * q.v[i] + q.s * this.v[i];
		this.s = _s;
		return this;
	}
	...
	public Quaternion interpolate(float t, Quaternion q1, Quaternion q2) {
		...
		float sinTheta = (float) Math.pow(1 - dp * dp, 2);

		float term1 = (float) Math.sin(theta * (1 - t)) / (float) Math.sin(theta);
		float term2 = (float) Math.sin(theta * t) / (float) Math.sin(theta);

		// q(t) = term1*q1 + term2*q2

		Quaternion rval = new Quaternion(new float[] { term1 * q1.v[0] + term2 * q2.v[0],
				term1 * q1.v[1] + term2 * q2.v[1], term1 * q1.v[2] + term2 * q2.v[2] });

		rval.s = term1 * q1.s + term2 * q2.s;
		return rval;
	}

	public Quaternion interpolate(float t, Quaternion q2) {
