...
public class PositionTool {
	...
	public PositionTool(Matrix _lookAt, Matrix _frameI, Matrix _frameJ, Matrix component, Matrix pivotPoint,
		...
		l3 = new Matrix( new float[] { cameraRotationArm.M[0], cameraRotationArm.M[1], cameraRotationArm.M[2] }, 3,
				1);
		...
		System.out.println("lookAt mag: " + lookAt.mag());
		System.out.println("frameJ mag: " + frameI.mag());
		System.out.println("frameK mag: " + frameJ.mag());
		System.out.println("");
		...
		l2_l1 = (float) Math.acos(l1.dot(l2) / l1.mag());

		l3_l2 = (float) Math.sin(l2_l1) * l1.mag() / l3.mag();
		l3_l2 = (float) Math.asin(l3_l2);

		l3_l1 = (float) Math.PI - l3_l2 - l2_l1;

		float scale = l3.mag() * (float) Math.sin(l3_l1) / (float) Math.sin(l2_l1);
		...
		adjustTheta = (float) Math.acos(lla.dot(llb));
		...
		System.out.println("l2_l1 = " + l2_l1);
		System.out.println("l3_l2 = " + l3_l2);
		System.out.println("l3_l1 = " + l3_l1);
		System.out.println("");
		System.out.println("adjustTheta = " + adjustTheta);
		System.out.println("");
		System.out.println("l3 mag = " + l3.mag());
		System.out.println("l3 adjust mag = " + l3_adjust.mag());
		...
		float dAng = (float) Math.acos(lookAt_copy.dot(l2_copy)); // angle between lookAt and component
		...
		float angCorrect = (float) Math.acos(frameI_copy.dot(planeIntersection));
	...
	Matrix getFullTransformation() {
		...
		for (int j = 0; j < 3; j++) {
			...
			for (int i = 0; i < 3; i++)
				fullTransform.M[j * 4 + i] = cRotate.M[j * 3 + i];
		}
